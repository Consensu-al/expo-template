...
export function useValidation<T extends z.ZodType>(
  ...
  const setValue = useCallback(
    <K extends keyof z.infer<T>>(field: K, value: z.infer<T>[K] | undefined) => {
  ...
  const resetForm = useCallback(
    (newValues: Partial<z.infer<T>> = initialValues) => {
      setValues(newValues);
      setErrors(null);
  ...
  const hasError = useCallback(
    (field: keyof z.infer<T>) => {
      return !!(errors?.[field as string] && touchedFields[field as string]);
    },
